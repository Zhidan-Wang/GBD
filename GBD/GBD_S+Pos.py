...
def train_and_eval(train_data, val_data, test_data, device = torch.device("cuda" if torch.cuda.is_available() else "cpu"),
    ...
    train_dense = train_rating.to_dense().cpu()
    ...
    discriminator_pos = Discriminator_POS(dis_model_args['emb_dim'], n_position).to(device)
    dis_optimizer_pos = torch.optim.SGD(discriminator_pos.parameters(), lr=dis_model_args['learning_rate'],
                                      weight_decay=0)
    ...
    for epo in range(early_stopping.max_epochs):
        ...
        for u_batch_idx, users in enumerate(train_loader.User_loader):
            ...
            #print("grad11111111:", base_model.item_latent.weight.grad)
            ...
            disturb1 = 0.4 * base_model_args['disturb_intensity'] * norm_disturb1
            ...
            disturb2 = 0.4 * base_model_args['disturb_intensity'] * norm_disturb2
            base_model.user_latent.weight.data = base_model.user_latent.weight.data - disturb2

            user_latent = base_model.user_latent(users_train)
            item_latent = base_model.item_latent(items_train)
            pos_preds = discriminator_pos(user_latent, item_latent).to(device).to(torch.float32)
            pos_labels = positions_train
            dis_loss_pos = pos_criterion(pos_preds, pos_labels)
            dis_loss_pos = dis_loss_pos.to(torch.float32)
            #print("pop_loss!!!", dis_loss_pos)

            dis_loss_pos.backward(retain_graph=True)
            #print("grad222222:", base_model.item_latent.weight.grad)
            attack_disturb_pos1 = base_model.item_latent.weight.grad.detach_()
            norm_pos1 = attack_disturb_pos1.norm(dim=-1, p=2)
            norm_disturb_pos1 = attack_disturb_pos1 / (norm_pos1.unsqueeze(dim=-1) + 1e-10)
            disturb_pos1 = 0.6 * base_model_args['disturb_intensity'] * norm_disturb_pos1
            # print("disturb_pop:::", disturb_pop)
            base_model.item_latent.weight.data = base_model.item_latent.weight.data - disturb_pos1

            attack_disturb_pos2 = base_model.user_latent.weight.grad.detach_()
            norm_pos2 = attack_disturb_pos2.norm(dim=-1, p=2)
            norm_disturb_pos2 = attack_disturb_pos2 / (norm_pos2.unsqueeze(dim=-1) + 1e-10)
            disturb_pos2 = 0.6 * base_model_args['disturb_intensity'] * norm_disturb_pos2
            base_model.user_latent.weight.data = base_model.user_latent.weight.data - disturb_pos2
            ...
            if train_f == 4:
                ...
                discriminator_pos.train()
                pos_preds = discriminator_pos(user_latent, item_latent).to(device).to(torch.float32)
                pos_labels = positions_train
                dis_loss2 = pos_criterion(pos_preds, pos_labels)
                dis_loss2 = dis_loss2.to(torch.float32)

                dis_loss2 = dis_loss2.to(torch.float32)

                dis_optimizer_pos.zero_grad()
                dis_loss2.backward()
                dis_optimizer_pos.step()
