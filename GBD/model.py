...
class Discriminator_S(nn.Module):
    def __init__(self, n_user, n_item, dim=64):
        super().__init__()
        self.emb_size = dim
        self.hidden_size = int(dim/2)

        self.linear1 = nn.Linear(self.emb_size, self.hidden_size)
        self.linear2 = nn.Linear(self.emb_size, self.hidden_size)


    def forward(self, user_latent, item_latent):

        u_latent = self.linear1(user_latent)
        i_latent = self.linear2(item_latent)

        preds = torch.mm(u_latent, i_latent.transpose(0,1))
        return preds

class Discriminator_P(nn.Module):
    def __init__(self, n_item, dim=64, n_position = 2, init = None):
        super().__init__()
        self.emb_size = dim
        self.p_class = n_position
        self.hidden_size = int(dim/2)
        ...
        self.linear1 = nn.Linear(self.emb_size, self.hidden_size)
        self.linear2 = nn.Linear(self.hidden_size, self.p_class)
        self.sigmoid = nn.Sigmoid()

        self.loss_fct = nn.CrossEntropyLoss()

    def forward(self, item_latent):
        i_latent = self.linear1(item_latent)
        preds = self.linear2(i_latent)
        preds = self.sigmoid(preds)
        #print("pred!!!", preds.shape)
        return preds.squeeze(dim=-1)

    def calculate_loss(self, item_latent, p_label):
        pred = self.forward(item_latent)
        loss = self.loss_fct(pred, p_label)
        return loss

class Discriminator_POS(nn.Module):
    def __init__(self, dim=64, n_position = 2):
        super().__init__()
        self.emb_size = dim
        self.p_class = n_position
        self.hidden_size = int(dim/2)
        ...
        self.linear1 = nn.Linear(self.emb_size, self.hidden_size)
        self.linear2 = nn.Linear(self.emb_size, self.p_class)
        self.sigmoid = nn.Sigmoid()
    ...
    def forward(self, user_latent, item_latent):
        i_latent = self.linear1(item_latent)
        i_latent = self.act(i_latent)
        u_latent = self.linear1(user_latent)
        u_latent = self.act(u_latent)
        latent = torch.cat((u_latent, i_latent), 1)

        preds = self.linear2(latent)
        preds = self.sigmoid(preds)
        return preds.squeeze(dim=-1)

    def calculate_loss(self, user_latent, item_latent, p_label):
        pred = self.forward(user_latent, item_latent)
        loss = self.loss_fct(pred, p_label)
        return loss
