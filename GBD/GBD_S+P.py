...
def para(args): 
    if args.dataset == 'yahooR3': 
        ...
        args.training_details_args = {'pop_thresh': 600, 'weight_s': 0.8}
        ...
        args.training_details_args = {'pop_thresh': 150, 'weight_s': 0.9}
    else: 
...
def train_and_eval(train_data, val_data, test_data, device = 'cuda',
        ...
        training_details_args: dict = {'pop_threash': 600, 'weight_s': 0.8}):
    ...
    item_pop = np.sum(train_dense, axis= 0)
    #print("pop::::", item_pop)
    pop_sort = np.sort(item_pop, axis=-1, kind='quicksort', order=None)

    pop_thresh = int(training_details_args['pop_thresh'])
    print("pop_thresh!!!", pop_thresh)
    pop_threshold = pop_sort[pop_thresh]
    pop_label = np.where(item_pop >= pop_threshold, 1, 0)
    # data shape
    ...
    discriminator_p = Discriminator_P(n_item, dis_model_args['emb_dim'], 2).to(device)
    dis_optimizer_p = torch.optim.SGD(discriminator_p.parameters(), lr=dis_model_args['learning_rate'],
                                      weight_decay=0)
    
    # loss_criterion
    criterion = nn.MSELoss(reduction='sum')
    none_criterion = nn.MSELoss(reduction='mean')
    pop_criterion = nn.CrossEntropyLoss()
    # begin training
    ...
    for epo in range(early_stopping.max_epochs):
        training_loss = 0
        discriminator_loss1 = 0
        discriminator_loss2 = 0
        train_f = 0
        for u_batch_idx, users in enumerate(train_loader.User_loader): 
            for i_batch_idx, items in enumerate(train_loader.Item_loader):
                ...
                #selec_label = np.where(train_dense != 0, 1, -1)
                selec_labels = torch.from_numpy(train_dense).to(device).to(torch.float32)
                dis_loss_s = none_criterion(selec_pred, selec_labels)
                dis_loss_s = dis_loss_s.to(torch.float32)
                dis_loss_s.backward(retain_graph=True)

                # step2: calculate perturbations
                #print("grad:", base_model.item_latent.weight.grad)
                weight_s = training_details_args['weight_s']
                ...
                disturb1 = weight_s * base_model_args['disturb_intensity'] * norm_disturb1
                ...
                disturb2 = weight_s * base_model_args['disturb_intensity'] * norm_disturb2
                base_model.user_latent.weight.data = base_model.user_latent.weight.data - disturb2

                item_latent = base_model.item_latent(items)
                pop_preds = discriminator_p(item_latent).to(device).to(torch.float32)
                pop_labels = pop_label[items.tolist()]
                pop_labels = torch.from_numpy(pop_labels).to(device)
                #dis_loss_p = discriminator_p.calculate_loss(item_latent, pop_labels)
                dis_loss_p = pop_criterion(pop_preds, pop_labels)
                dis_loss_p = dis_loss_p.to(torch.float32)
                #print("pop_loss!!!", dis_loss_p)

                dis_loss_p.backward(retain_graph=True)
                #print("grad:", base_model.item_latent.weight.grad)
                attack_disturb_pop = base_model.item_latent.weight.grad.detach_()
                norm_pop = attack_disturb_pop.norm(dim=-1, p=2)
                norm_disturb_pop = attack_disturb_pop / (norm_pop.unsqueeze(dim=-1) + 1e-10)
                disturb_pop = (1 - weight_s) * base_model_args['disturb_intensity'] * norm_disturb_pop * 10
                #print("disturb_pop:::", disturb_pop)
                base_model.item_latent.weight.data = base_model.item_latent.weight.data - disturb_pop
                ...
                if train_f == 4:
                    ...
                    discriminator_loss1 += dis_loss1.item()

                    discriminator_p.train()
                    pop_preds = discriminator_p(item_latent).to(device).to(torch.float32)
                    pop_labels = pop_label[items.tolist()]
                    pop_labels = torch.from_numpy(pop_labels).to(device)
                    dis_loss2 = pop_criterion(pop_preds, pop_labels)
                    dis_loss2 = dis_loss2.to(torch.float32)

                    dis_optimizer_p.zero_grad()
                    dis_loss2.backward()
                    dis_optimizer_p.step()

                    discriminator_loss2 += dis_loss2.item()
...
if __name__ == "__main__": 
    ...
    train_and_eval(train, validation, test, device, base_model_args = args.base_model_args, dis_model_args = args.dis_model_args, training_args = args.training_args,
                   training_details_args = args.training_details_args)
